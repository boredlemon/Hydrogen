/*
** $Id: cupc.c $
** Cup compiler (saves bytecodes to files; also lists bytecodes)
...
#define cupU_print	PrintFunction

#define PROGNAME	"cupc"		/* default program name */
...
static const char* reader(cup_State* L, void* ud, size_t* size)
{
 UNUSED(L);
 if ((*(int*)ud)--)
 {
...
static const Proto* combine(cup_State* L, int n)
{
 if (n==1)
  return toproto(L,-1);
 else
 {
  Proto* f;
  int i=n;
  if (cup_load(L,reader,&i,"=(" PROGNAME ")",NULL)!=CUP_OK) fatal(cup_tostring(L,-1));
  f=toproto(L,-1);
  for (i=0; i<n; i++)
  {
...
static int writer(cup_State* L, const void* p, size_t size, void* u)
{
 UNUSED(L);
...
static int pmain(cup_State* L)
{
 ...
 int i;
 tmname=G(L)->tmname;
 if (!cup_checkstack(L,argc)) fatal("too many input files");
 for (i=0; i<argc; i++)
 {
  const char* filename=IS("-") ? NULL : argv[i];
  if (cupL_loadfile(L,filename)!=CUP_OK) fatal(cup_tostring(L,-1));
 }
 f=combine(L,argc);
 if (listing) cupU_print(f,listing>1);
 if (dumping)
 {
  FILE* D= (output==NULL) ? stdout : fopen(output,"wb");
  if (D==NULL) cannot("open");
  cup_lock(L);
  cupU_dump(L,f,writer,D,stripping);
  cup_unlock(L);
...
int main(int argc, char* argv[])
{
 ...
 cup_pushcfunction(L,&pmain);
 cup_pushinteger(L,argc);
 cup_pushlightuserdata(L,argv);
 if (cup_pcall(L,2,0,0)!=CUP_OK) fatal(cup_tostring(L,-1));
 cup_close(L);
 return EXIT_SUCCESS;
}
...
static void PrintConstant(const Proto* f, int i)
{
 ...
 switch (ttypetag(o))
 {
  ...
  case CUP_VNUMFLT:
	{
	char buff[100];
	sprintf(buff,CUP_NUMBER_FMT,fltvalue(o));
	printf("%s",buff);
	if (buff[strspn(buff,"-0123456789")]=='\0') printf(".0");
	break;
	}
  case CUP_VNUMINT:
	printf(CUP_INTEGER_FMT,ivalue(o));
	break;
  case CUP_VSHRSTR:
  case CUP_VLNGSTR:
...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  int isk=GETARG_k(i);
  int line=cupG_getfuncline(f,pc);
  printf("\t%d\t",pc+1);
